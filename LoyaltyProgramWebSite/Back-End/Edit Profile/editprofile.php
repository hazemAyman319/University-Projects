  ...
  $id = $row['customer_id'];
    ...
    if(isset($_POST["submit"])){
        ...
        if(empty($first_name) || empty($last_name) || empty($emaill) || empty($phone) || empty($type)){
            ...
            if ($emaill==$email){
                $sql = "UPDATE `customer` SET `first_name`='$first_name', `last_name`='$last_name', `email`='$emaill', `phone`='$phone', `customer_type`='$type' WHERE `customer_id`='$id'";
                $result = mysqli_query($conn, $sql);
                ...
                $select = " SELECT * FROM customer WHERE email = '$emaill' ";
